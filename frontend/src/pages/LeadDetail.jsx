...
function QuotationRecordCard({ quotation, onSend }) {
  ...
  const download = () => {
    const doc = buildDynamicQuotationPdf(quotation);
  ...
  return (
    <div className="panel p-5">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          <p className="font-display text-base font-extrabold">
            {quotation.quotation_number} · {quotation.quotation_type || "Quotation"}
          </p>
          <p className="mt-1 text-sm text-muted-foreground">
            {quotation.subject || "Quotation"} · {currency(Number(quotation.total_amount || 0))}
          </p>
  ...
  return (
    ...
    >
      ...
      <Section
        ...
        action={
          <div className="flex items-center gap-2">
            <AddButton onClick={() => setQuotationBuilderOpen(true)}>
              New Quotation
            </AddButton>
          </div>
        }
